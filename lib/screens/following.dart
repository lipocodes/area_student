...
class _FollowingsState extends State<Followings> {
  ...
  List followings = [];
  List followingUid = [];
  List followingFirstName = [];
  List followingLastName = [];
  List followingProfileImage = [];
  bool tempUid = false;
  ...
  getDataOfUsers(String uid) async {
    ...
    try {
      this.followingUid.add(snapshot[0].data['uid']);
      this.followingFirstName.add(snapshot[0].data['firstName']);
  ...
  void listFollowings() async {
    ...
    final QuerySnapshot result = await Firestore.instance
        ...
        .getDocuments();
    final List<DocumentSnapshot> snapshot = result.documents;
    this.followings = snapshot[0].data['following'];

    for (int i = 0; i < this.followings.length; i++) {
      await getDataOfUsers(this.followings[i]);
    }

    setState(() {});
  }

  removeFollowing(index) async {
    this.followings.removeAt(index);
    await firestore
        .collection("userData")
        .document(this.uid)
        .updateData(({'following': followings}))
        .whenComplete(() {
      followings = [];
      followingUid = [];
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: ListView.builder(
                  itemCount: followingFirstName.length,
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          ...
                          SizedBox(
                            width: 200.0,
                            child: new Text(
                              followingFirstName[index] != null
                                  ? followingFirstName[index] +
                                      " " +
                                      followingLastName[index]
                                  : "",
                              style: new TextStyle(
                                fontSize: 20.0,
                                fontFamily: "Poppins",
